...
export class EmployeeService {
  ...
  addEmployee(employee: Omit<Employee, 'personId' | 'employeeNum'> & { password: string }): Observable<Employee> {
    const newPersonId = this.employees.length > 0 ? Math.max(...this.employees.map(e => e.personId)) + 1 : 1;
